...
namespace te
{
    TeModel::TeModel(TeDevice &device, const std::vector<Vertex> &vertices) : teDevice{device}
    {
    ...
    void TeModel::createVertexBuffers(const std::vector<Vertex> &vertices)
    {
        ...
        void *data;
        vkMapMemory(teDevice.device(), vertexBufferMemory, 0, bufferSize, 0, &data);
        memcpy(data, vertices.data(), static_cast<size_t>(bufferSize));
        vkUnmapMemory(teDevice.device(), vertexBufferMemory);
    }
